 ...
 * 
 * This implementation is specific for int keys and float values, avoiding boxing and unboxing 
 * of those primitive types, leading to a better running time compared to the generic version.
 *  
 ...
 */
public class IntFloatTreap implements Iterable<IndexedFloat>
{
	private IntFloatTree tree;
	...
	public synchronized void put(int key, float value) {
		IntFloatTree node = new IntFloatTree();
		node.key = key;
		node.value = value;
	 ...
	 * @return an iterator.
	 */
	public synchronized Iterator<IndexedFloat> iterator() {
		return new IntFloatTreapIterator(tree,true);
	}
	 ...
	 * @return an iterator.
	 */
	public synchronized Iterator<IndexedFloat> iterator(boolean ascending) {
		return new IntFloatTreapIterator(tree,ascending);
	}
	...
	private IntFloatTree insert(IntFloatTree node, IntFloatTree tree) {
	 ...
	 */
	private IntFloatTree delete(int key, IntFloatTree t) {
...
class IntFloatTree
{
	...
	float value;
	int prio;
	IntFloatTree left, right;

	/* Rotate this tree to the left
	 */
	final IntFloatTree rotateLeft() {
		IntFloatTree temp = right;
		right = right.left;
		temp.left = this;
		return temp;
	}
	 ...
	 */
	final IntFloatTree rotateRight() {
		IntFloatTree temp = left;
		left = left.right;
		temp.right = this;
		return temp;
	}
 ...
 */
class IntFloatTreapIterator implements Iterator<IndexedFloat>
{
	private Stack<IntFloatTree> stack;
	
	private boolean ascending;

	IntFloatTreapIterator(IntFloatTree t, boolean ascending) {
		stack = new Stack<IntFloatTree>();
	...
	public IndexedFloat next() {
		...
		IntFloatTree node =  stack.peek();

		if (ascending)
			if (node.right == null) {
				IntFloatTree t =  stack.pop();
				while (!stack.empty() && ( stack.peek()).right == t)
					t =  stack.pop();
			} 
			else
				for (IntFloatTree t = node.right; t != null; t = t.left)
					stack.push(t);
		else // descending
			if (node.left == null) {
				IntFloatTree t =  stack.pop();
				...
				for (IntFloatTree t = node.left; t != null; t = t.right)
					stack.push(t);

		return new IndexedFloat(node.key,node.value);
	}
